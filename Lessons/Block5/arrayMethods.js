...
-Переедает\Недоедает - означает что собака ест больше рекомендованной порции или меньше рекомендованной на 10%
...
//1
dogs.forEach((val) => val.recommendedFood = Math.floor(val.weight ** 0.75 * 28));
//console.log(dogs);

//2
const dogMarina = dogs.find((own) => own.owners.includes('Marina'));
console.log(`Собака Марини їсть ${dogMarina.curFood > dogMarina.recommendedFood ? "забагато" : "замало"} `);
...
//4
console.log(`Собаки ${ownersEatTooLittle.join(" і ")} їдять замало`);
...
//7
const dogsNorma = dogs.filter((dog) =>
  dog.curFood > dog.recommendedFood * 0.9 &&
  dog.curFood < dog.recommendedFood * 1.1);
console.log(dogsNorma);
